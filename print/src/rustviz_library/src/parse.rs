...
pub fn parse_vars_to_map(main_rs: &str) -> Result<(
    std::str::Lines, u64, HashMap<String, ResourceAccessPoint> )>
{
    // read main file
    let mut fin_lines = main_rs.lines();

    // check for unchanged template
    let mut line = fin_lines.next().ok_or_else(|| anyhow!("Oops, could not read. Empty main.rs string maybe?"))?;
    if line != "/* --- BEGIN Variable Definitions ---" {
        return Err(anyhow!(format!("Uh oh! Do not change the first line! Line: {} ", line)));
    }
    ...
    while {
        line = fin_lines.next().ok_or_else(|| anyhow!("Something went wrong! Do not remove BEGIN and END statements!"))?;
    ...
    let rap_map = vec_to_map(vars)?;
    Ok((fin_lines, num_lines, rap_map))
}
...
//          {key, value} pair = {name, ResourceAccessPoint}
fn vec_to_map(vars_str: Vec<String>) -> Result<HashMap<String, ResourceAccessPoint>> {
    ...
    for v in vars_str.iter() {
        ...
        if fields.is_empty() || fields.len() < 2 {
            return Err(anyhow!(var_usage_error(&fields)));
        }
...
pub fn extract_events(
    ...
    for (lnum, line) in fin_lines.enumerate() {
        ...
        if block { // if searching inside block comment
            ...
            if let Some(_) = line_string.find("!{") {
                return Err(anyhow!(delimitation_err(line_begin+main_line)));
            }
    ...
    if block { return Err(anyhow!(delimitation_err(line_begin+main_line))); }
...
) -> Result<Option<ResourceAccessPoint>> {
    if name == "None" { Ok(None) }
    else {
        match vars.get(name) {
            Some(res) => Ok(Some(res.clone())),
            None => {
              return Err(anyhow!(format!("Variable '{}' does not exist! Name must match definition", name))); 
            }
        }
    }
}
...
fn get_mut_qualifier(fields: &Vec<&str>) -> Result<bool> {
    if fields.len() == 2 { Ok(false) }
    else if fields[1] == "mut" { Ok(true) }
    else {
      return Err(anyhow!(format!("Did not understand qualifier '{}' of variable '{}'! Field must either be empty or 'mut'.",
      fields[1], fields[2])));
    }
}
    ...
    while idx < fields.len() {
        *hash += 1;
        ...
        let v_name = parent_name.clone() + "." + (
            if cond {
                if idx+1 >= fields.len() {
                    return Err(anyhow!("Expected variable name after 'mut' qualifier, found nothing!"));
                }
                fields[idx+1]
            } else {
                fields[idx]
...
// Requires: Nothing
// Modifies: Nothing
// Effects: Returns error message string
fn var_usage_error(fields: &Vec<&str>) -> String {
    format!("Incorrect variable formatting '{}'!\
...
fn delimitation_err(line_num: u64) -> String{
    format!(
